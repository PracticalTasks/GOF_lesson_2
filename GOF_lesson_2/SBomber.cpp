...
void SBomber::DropBomb()
{
	if (bombsNumber > 0)
	{
		FileLoggerSingleton::getInstance().WriteToLog(std::string(__FUNCTION__) + " was invoked");
...
void SBomber::DropNBomb()
{
	if (bombsNumber > 0)
	{
		...
		pCommand = new DropNBombCommand(pPlane, vecDynamicObj,bombsNumber, score);
		pCommand->execute();
	}
}
